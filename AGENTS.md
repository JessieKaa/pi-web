...
`GET /api/models` returns `defaultModel` and per-model `initialThinkingLevels`, which preview the read-only `defaultThinkingLevel` (with scope pins and per-model overrides). `ChatWindow` pre-selects the default model on mount for new sessions. Explicit browser model/thinking selections are applied atomically during AgentSession construction; `lib/startup-preferences.ts` persists only an explicitly selected model. Thinking levels stay session-local: pi-web never writes the global `defaultThinkingLevel`, and implicit `enabledModels` fallbacks and thinking pins are not persisted. Existing-session model switches retain their separate behavior of choosing the model's highest supported thinking level (or its scope pin).
