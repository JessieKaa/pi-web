...
function createHarness({ readinessTimeoutMs = 100, reconnectDelayMs = 5, staleAfterMs, now } = {}) {
  ...
  const connection = new AgentEventConnection({
    createSource(sessionId, { startsRuntime }) {
      const source = new FakeEventSource();
      source.sessionId = sessionId;
      source.startsRuntime = startsRuntime;
      sources.push(source);
      return source;
    },
...
test("upgrades an observer transport before an explicit runtime start", async () => {
  const { connection, sources } = createHarness();
  const observing = connection.ensureConnected("session-a");
  sources[0].open();
  sources[0].message({ type: "connected" });
  await observing;
  assert.equal(sources[0].startsRuntime, false);

  const starting = connection.ensureConnected("session-a", { startsRuntime: true });
  await nextTurn();
  assert.equal(sources.length, 2);
  assert.equal(sources[0].closeCount, 1);
  assert.equal(sources[1].startsRuntime, true);
  sources[1].open();
  sources[1].message({ type: "connected" });
  await starting;

  // An explicitly-startable source is still safe to keep as an observer.
  await connection.ensureConnected("session-a");
  assert.equal(sources.length, 2);
});
