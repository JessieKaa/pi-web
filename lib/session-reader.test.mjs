...
test("renders the raw parent-chain transcript across compaction", () => {
  const entries = [
    ...
    userEntry("u2", "a1", "kept request"),
    {
      ...
      summary: "summary", firstKeptEntryId: "u2", tokensBefore: 100,
    },
    userEntry("u3", "cmp", "new request"),
    userEntry("side", "a1", "side request"),
  ];

  const main = buildSessionTranscript(entries, "u3");
  assert.deepEqual(main.entryIds, ["u1", "a1", "u2", "cmp", "u3"]);
  assert.equal(main.messages[3].customType, "compaction");
  assert.equal(main.messages[3].content, "summary");

  const side = buildSessionTranscript(entries, "side");
  assert.deepEqual(side.entryIds, ["u1", "a1", "side"]);
});
...
test("strips thinkingSignature only from deferred browsing DTOs", () => {
  const entries = [
    userEntry("u1", null, "start"),
    {
      ...assistantEntry("a1", "u1", "answer"),
      message: {
        ...
        model: "test-model",
        content: [
          { type: "thinking", thinking: "large reasoning", thinkingSignature: "sig-abc" },
          { type: "text", text: "answer" },
  ...
  for (const build of [buildSessionContext, buildSessionTranscript]) {
    const deferred = build(entries, undefined, { deferThinking: true });
    const content = deferred.messages[1].content;

    // The deferred thinking block survives at the same ordinal, empty and
    // flagged, with the provider signature removed by explicit projection.
    assert.deepEqual(content[0], { type: "thinking", thinking: "", deferred: true });
    assert.equal("thinkingSignature" in content[0], false);
    // Empty thinking blocks are not deferred and keep their own fields.
    assert.deepEqual(content[2], { type: "thinking", thinking: "", thinkingSignature: "sig-empty" });
    // Block ordinals/count and entry path are unchanged.
    assert.equal(content.length, 3);
    assert.deepEqual(deferred.entryIds, ["u1", "a1"]);

    // The non-defer build preserves the signature for the same entry.
    const full = build(entries, undefined, {});
    assert.equal(full.messages[1].content[0].thinking, "large reasoning");
    assert.equal(full.messages[1].content[0].thinkingSignature, "sig-abc");
  }

  // Neither pass mutates the input entries or their content blocks.
...
test("readSessionTranscriptWindow pages through compaction without dropping old messages", () => {
  const dir = mkdtempSync(join(tmpdir(), "pi-web-transcript-window-"));
  const filePath = join(dir, "chat.jsonl");
  const entries = [
    { type: "session", version: 3, id: "transcript", timestamp: "2026-01-01T00:00:00.000Z", cwd: dir },
    userEntry("u1", null, "old request"),
    assistantEntry("a1", "u1", "old answer"),
    userEntry("u2", "a1", "kept request"),
    { type: "compaction", id: "cmp", parentId: "u2", timestamp: "2026-01-01T00:00:03.000Z", summary: "summary", firstKeptEntryId: "u2", tokensBefore: 100 },
    userEntry("u3", "cmp", "new request"),
  ];
  writeJsonl(filePath, entries);
  try {
    const newest = readSessionTranscriptWindow(filePath, { limit: 2 });
    assert.deepEqual(newest.context.entryIds, ["cmp", "u3"]);
    assert.equal(newest.hasMore, true);
    const older = readSessionTranscriptWindow(filePath, { limit: 3, before: "cmp" });
    assert.deepEqual(older.context.entryIds, ["u1", "a1", "u2"]);
    assert.equal(older.hasMore, false);
    const stale = readSessionTranscriptWindow(filePath, { limit: 3, before: "missing" });
    assert.equal(stale.historyChanged, true);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test("context edits and usage records stay out of the chat transcript", () => {
  const agentDir = mkdtempSync(join(tmpdir(), "pi-web-context-edit-"));
  const filePath = join(agentDir, "chat.jsonl");
  writeJsonl(filePath, [
    { type: "session", version: 3, id: "edit", timestamp: "2026-01-01T00:00:00.000Z", cwd: agentDir },
    userEntry("u1", null, "kept user request"),
    {
      type: "context_edit",
      id: "edit1",
    ...
    userEntry("u2", "use1", "after edit"),
  ]);
  const window = readSessionWindow(filePath, { limit: 10 });
  assert.deepEqual(window.context.entryIds, ["u1", "u2"]);
  assert.equal(window.context.messages[0].content, "kept user request");
  assert.equal(window.context.messages.some((message) => String(message.content).includes("warmed")), false);
});
