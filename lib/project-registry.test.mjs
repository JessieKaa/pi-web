...
test("creates a registry row when a session-only project is first updated", async () => {
  const directory = await mkdtemp(join(tmpdir(), "pi-web-project-registry-"));
  const registryPath = join(directory, "projects.json");
  const sessionOnlyPath = resolve(directory, "derived-from-sessions");
  await writeFile(registryPath, JSON.stringify({ version: 1, projects: [] }));

  try {
    await updateProjectPreference(sessionOnlyPath, { removed: true }, registryPath);
    const stored = JSON.parse(await readFile(registryPath, "utf8"));
    assert.deepEqual(stored.projects, [
      { path: sessionOnlyPath, pinned: false, archived: false, removed: true, order: 0 },
    ]);
  } finally {
