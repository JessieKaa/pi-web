...
test("RPC session startup injects the hidden inline extensions and the built-in subagent extension", async () => {
  ...
  assert.match(startupSource, /new AgentSessionWrapper\(inner, systemPromptOverride\)/);
  assert.match(startupSource, /createSystemPromptOverrideExtension\(systemPromptOverride\)/);
...
test("RPC session startup persists only an explicit model and keeps thinking session-local", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const preferences = await readFile(new URL("./startup-preferences.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  const startupCall = startupSource.slice(startupSource.indexOf("persistExplicitStartupPreferences("), startupSource.indexOf("if (persistedPreferences.modelDefaultChanged)"));

  assert.match(startupCall, /initialModel \? \{ model: initialModel \}/);
  assert.doesNotMatch(startupCall, /\{ thinkingLevel \}/);
  assert.doesNotMatch(preferences, /setDefaultThinkingLevel/);
  assert.match(startupSource, /modelDefaultChanged\) invalidateModelsCache\(\)/);
  assert.match(source, /this\.inner\.setThinkingLevel\(level\)/);
  assert.doesNotMatch(source, /this\.inner\.setThinkingLevel\(level, \{ persist: true \}\)/);
});
...
test("reloading a session invalidates the models cache", async () => {
  ...
  assert.match(reloadSource, /this\.syncSystemPromptOverride\(\);\s*invalidateModelsCache\(\)/);
});
...
test("queue-wide interject steers every pending item FIFO per kind", async () => {
  ...
  const end = source.indexOf("  private syncSystemPromptOverride(", start);
  assert.ok(end > start, "steerAllQueued must still be followed by syncSystemPromptOverride");
