...
test("tool results do not consume the visible window", () => {
  const messages = [];
  const entryIds = [];
  for (let i = 0; i < 3; i++) {
    messages.push({ role: "user", content: `u${i}` });
    entryIds.push(`u${i}`);
    messages.push({ role: "toolResult", content: `t${i}` });
    entryIds.push(`t${i}`);
  }
  const page = sliceSessionContext(
    { messages, entryIds, thinkingLevel: "off", model: null },
    { limit: 2 },
  );
  assert.deepEqual(page.context.entryIds, ["u1", "t1", "u2", "t2"]);
  assert.equal(page.hasMore, true);
});

test("transcript pages by entries while context pages by visible messages", () => {
  const context = {
    messages: [
      { role: "user", content: "old" },
      { role: "toolResult", content: "old result" },
      { role: "assistant", content: "new" },
      { role: "toolResult", content: "new result" },
    ],
    entryIds: ["u1", "t1", "a1", "t2"],
    thinkingLevel: "off", model: null,
  };
  const transcript = sliceSessionContext(context, { limit: 2, mode: "entries" });
  const modelContext = sliceSessionContext(context, { limit: 2, mode: "visible" });
  assert.deepEqual(transcript.context.entryIds, ["a1", "t2"]);
  assert.equal(transcript.hasMore, true);
  assert.deepEqual(modelContext.context.entryIds, ["u1", "t1", "a1", "t2"]);
  assert.equal(modelContext.hasMore, false);
  assert.deepEqual(sliceSessionContext(context, { limit: 2, before: "a1", mode: "entries" }).context.entryIds, ["u1", "t1"]);
});
