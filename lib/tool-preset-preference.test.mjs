...
test("missing or invalid preferences inherit Pi's configured tools", () => {
  assert.equal(getPreferredToolPreset(createStorage()), null);
  assert.equal(
...
test("falls back safely when browser storage is unavailable", () => {
  ...
  assert.equal(getPreferredToolPreset(unavailable), null);
  assert.doesNotThrow(() => setPreferredToolPreset("full", unavailable));
  assert.equal(getPreferredToolPreset(null), null);
...
test("falls back when accessing window.localStorage throws", () => {
  ...
  try {
    assert.equal(getPreferredToolPreset(), null);
