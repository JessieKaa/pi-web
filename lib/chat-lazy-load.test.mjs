...
test("absorbs an explicit click's own observer leave/re-enter instead of loading older", async () => {
  ...
  let result = reduceSentinelPaging(state, { type: "click", planHasMore: true }, armedContext);
...
test("an explicit click expands from the render plan while the observer keeps the raw mixed count", async () => {
  const { createSentinelPagingState, reduceSentinelPaging } = await loadSubject();
  // plan.items.length > visibleCount so the button offers a local page, but
  // messages.length <= visibleCount so the observer's raw count says no more.
  const rawExhausted = pagingContext({ renderedHasMore: false, historyHasMore: true });

  const click = reduceSentinelPaging(
    createSentinelPagingState(),
    { type: "click", planHasMore: true },
    rawExhausted,
  );
  assert.equal(click.action, "expand");
  assert.notEqual(click.action, "load-older");

  // Once the plan is exhausted too, the click is allowed to fetch older rows.
  const clickExhausted = reduceSentinelPaging(
    createSentinelPagingState(),
    { type: "click", planHasMore: false },
    rawExhausted,
  );
  assert.equal(clickExhausted.action, "load-older");

  // The observer still pages from the raw source-message count.
  const state = reduceSentinelPaging(
    createSentinelPagingState(),
    { type: "observer", intersecting: false },
    rawExhausted,
  ).state;
  const enter = reduceSentinelPaging(state, { type: "observer", intersecting: true }, rawExhausted);
  assert.equal(enter.action, "load-older");
});

test("a programmatic scroll change without user intent keeps absorbing", async () => {
  const { createSentinelPagingState, reduceSentinelPaging } = await loadSubject();
  const exhausted = pagingContext();
  ...
  // The scroll restore fires a position change but no intent was recorded.
...
test("re-enables auto paging only after a genuine user leave and re-enter", async () => {
  ...
  state = reduceSentinelPaging(state, { type: "click", planHasMore: true }, armedContext).state;
  ...
  // A real user scroll cycle (intent + position change) closes the absorbing
  // window; the sentinel is still on screen so the scroll alone does not arm it.
  state = reduceSentinelPaging(state, { type: "user-scroll-intent" }, exhausted).state;
  state = reduceSentinelPaging(state, { type: "user-scroll" }, exhausted).state;
...
test("a later explicit click still pages immediately after an absorbed cascade", async () => {
  ...
  const result = reduceSentinelPaging(state, { type: "click", planHasMore: false }, exhausted);
  assert.equal(result.action, "load-older");
  assert.equal(result.state.phase, "absorbing");
});
...
test("does not reopen the cascade for user scrolls while an older page is loading", async () => {
  const { createSentinelPagingState, reduceSentinelPaging } = await loadSubject();
  const loading = pagingContext({ loadingOlderHistory: true });
  const exhausted = pagingContext();
  ...
  // The user keeps scrolling while the request is in flight. Neither the intent
  // nor the position change may close the absorbing window.
  state = reduceSentinelPaging(state, { type: "user-scroll-intent" }, loading).state;
  state = reduceSentinelPaging(state, { type: "user-scroll" }, loading).state;
  assert.equal(state.phase, "absorbing");

  let result = reduceSentinelPaging(state, { type: "observer", intersecting: false }, exhausted);
  assert.equal(result.action, "none");
  result = reduceSentinelPaging(result.state, { type: "observer", intersecting: true }, exhausted);
  assert.equal(result.action, "none");
  assert.equal(result.state.phase, "absorbing");

  // After loading settles a genuine cycle closes the window and paging resumes.
  state = reduceSentinelPaging(result.state, { type: "user-scroll-intent" }, exhausted).state;
  state = reduceSentinelPaging(state, { type: "user-scroll" }, exhausted).state;
  assert.equal(state.phase, "idle");
  state = reduceSentinelPaging(state, { type: "observer", intersecting: false }, exhausted).state;
  result = reduceSentinelPaging(state, { type: "observer", intersecting: true }, exhausted);
  assert.equal(result.action, "load-older");
});
