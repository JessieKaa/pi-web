...
test("retains a child's resolved primary id when the primary is absent from this inventory", () => {
  const related = attachSessionRelations([
    session("worker", "subagent-worker-317e1ca0-1", "root"),
  ]);

  assert.equal(related[0].sessionRole, "subagent");
  assert.equal(related[0].rootSessionId, "root");
});
