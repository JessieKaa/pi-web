...
test("contains chat content and inputs within the mobile viewport", () => {
  ...
  assert.match(chatInputSource, /flex: "0 1 auto",\s*minWidth: 0,\s*width: "100%",[\s\S]*?maxHeight: 200,/);
});
...
test("keeps the composer symmetric now that the minimap sits outside the column", () => {
  ...
  assert.doesNotMatch(chatWindowSource, /contextGutter/);
  assert.match(chatWindowSource, /className="chat-workspace-main"[\s\S]*?<ChatMinimap/);
});
...
test("hides the extension status shelf while the mobile keyboard is open", () => {
  ...
  assert.doesNotMatch(chatWindowSource, /gutterDuplicate/);
